<template>
  <div class="commodity-collect">
    <navBox class="nav-box">
      <NavBar left-arrow title="文章标题" @click-left="pageReturn" >
      </NavBar>
    </navBox>
    <div class="container-box">
      <h1 class="h1-title">{{ bookData.title }}</h1>
      <div class="describe">
        <span class="item">
          <van-icon class="icon" name="eye-o" color="#9f9f9f" size="15" />5454
        </span>
        <span class="item">
          <van-icon class="icon" name="notes-o" color="#9f9f9f" size="15" />{{ bookData.createTime }}
        </span>
      </div>
      <div class="html-box" v-html="bookData.html"></div>
    </div>
  </div>
</template>
...
import { NavBar, showToast, Icon as vanIcon } from 'vant';
// @ts-ignore
...
/* 书数据 */
const bookData = reactive(bookListData[route.query.id]);
