<template>
  <div class="education-list">
    <navBox class="nav-box">
      <NavBar left-arrow title="操作指南" @click-left="pageReturn">
      </NavBar>
    </navBox>
    <!-- 下拉刷新上拉加载 -->
    <PullRefresh class="container-box" v-model="refreshState" @refresh="onRefresh">
      <List v-model:loading="loadingState" :finished="isFinished" finished-text="没有更多了" @load="onLoad" >
        <instructionsDetailItem v-for="item in 20" :key="item" />
      </List>
    </PullRefresh>
  </div>
</template>
...
import instructionsDetailItem from './components/instructionsDetailItem.vue';
